...
반환 결과에는 두 가지 값이 있습니다.

* 확인(기본값)
* 취소: 일반적으로 취소 버튼의 팝업 닫기 명령은 반환 결과를 취소로 설정해야 합니다. 닫기 버튼 클릭, ESC 키 누르기, 회색 영역 클릭 등으로 페이지를 닫으면 포건시는 팝업 페이지에서 취소 결과가 포함된 클릭 가능한 셀을 찾아 모든 명령을 실행합니다. 이런 방식으로 사용자는 팝업 페이지를 닫는 논리를 제어할 수 있습니다.
