...
외부 데이터베이스에서 데이터 테이블 만들기를 지원합니다. 만든 데이터 테이블은 기본 dbo패턴을 사용합니다.
...
\[파일]>\[옵션]>\[응용 프로그램 설정]>\[기타] > \[리스트뷰에서 표시되는 연결된 테이블의 최대 행 수] 0보다 큰 정수 또는 무제한을 설정하고 설정이 완료되면 연결된 테이블을 새로 고쳐 적용합니다.
...
데이터 원본을 선택한 후 데이터베이스에 연결하여 뷰 이름 및 SQL 문을 편집하여 뷰를 만들 수 있는 뷰 만들기 대화 상자를 엽니다.

![](../../.gitbook/assets/db118.png)

뷰를 만든 후 뷰를 열면 형식이 연결된 뷰를 볼 수 있으며 기본적으로 "Forguncy에서 데이터베이스 또는 테이블 스키마를 수정하도록 허용"을 선택합니다.
